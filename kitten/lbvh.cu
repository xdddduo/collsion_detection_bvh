...
#pragma region LBVHDevice
	...
	namespace LBVHKernels {
		...
		__global__ void translateAABBsKernel(Kitten::Bound<3, float>* aabbs, glm::vec3 offset, int numObjs) {
			int idx = threadIdx.x + blockIdx.x * blockDim.x;
			if (idx >= numObjs) return;
			aabbs[idx].min += offset;
			aabbs[idx].max += offset;
		}
...
#pragma region LBVH
	...
	void LBVH::translate(glm::vec3 offset) {
		LBVHKernels::translateAABBsKernel<<<(numObjs + 255) / 256, 256>>>(
			thrust::raw_pointer_cast(impl->d_objs),
			offset,
			numObjs
		);
		checkCudaErrors(cudaGetLastError());
		refit();  // Update internal nodes
	}
...
#pragma region testing
	...
	void printBVHTree(const thrust::host_vector<LBVH::node>& nodes, uint32_t idx, int depth = 0) {
		bool isLeaf = idx >> 31;
		idx &= 0x7FFFFFFF;
	
		std::string indent(depth * 2, ' ');
		if (isLeaf) {
			printf("%sLeaf AABB index: %u\n", indent.c_str(), idx);
			return;
		}
	
		if (idx >= nodes.size()) {
			printf("%s[ERROR] Invalid node index: %u (max: %zu)\n", indent.c_str(), idx, nodes.size());
			return;
		}
	
		const auto& n = nodes[idx];
		printf("%sNode %u: fence = %u\n", indent.c_str(), idx, n.fence);
		printf("%s  Left  (%s): %u\n", indent.c_str(), (n.leftIdx >> 31) ? "leaf" : "node", n.leftIdx & 0x7FFFFFFF);
		printf("%s  Right (%s): %u\n", indent.c_str(), (n.rightIdx >> 31) ? "leaf" : "node", n.rightIdx & 0x7FFFFFFF);
	
		// Traverse left
		if (n.leftIdx >> 31) {
			printf("%s    Leaf AABB index: %u\n", indent.c_str(), n.leftIdx & 0x7FFFFFFF);
		} else {
			printBVHTree(nodes, n.leftIdx, depth + 1);
		}
	
		// Traverse right
		if (n.rightIdx >> 31) {
			printf("%s    Leaf AABB index: %u\n", indent.c_str(), n.rightIdx & 0x7FFFFFFF);
		} else {
			printBVHTree(nodes, n.rightIdx, depth + 1);
		}
	}
	...
	void LBVH::bvhSelfCheck() const {
		...
		printf("BVH Tree:\n");
		printBVHTree(nodes, 0); // Start from root node
